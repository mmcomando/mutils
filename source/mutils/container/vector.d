...
import std.traits : Unqual, isCopyable, TemplateOf, isDynamicArray, hasMember;
...
struct Vector(T) {
	enum preAllocated = T.sizeof > 1024 ? 0 : 32;
	T[] heap;
	size_t used;
	static if (preAllocated > 0) {
		align(T.alignof) ubyte[(T[preAllocated]).sizeof] local;
	}
	...
	T[] array() {
		static if (preAllocated == 0) {
			...
			return (cast(T*) local.ptr)[0 .. preAllocated];
		}
	}
...
public:
	...
	void extend(size_t newNumOfElements) {
		T[] oldArray;
		T[] newArray;
		manualExtend22(oldArray, newArray, newNumOfElements);
		if (heap !is null && oldArray !is null) {
			freeData(cast(void[]) oldArray);
		}
		heap = newArray;
	}
	...
	void manualExtend22(ref T[] oldArray, ref T[] newArray, size_t newNumOfElements = 0) {
		...
		newArray = memory[0 .. newNumOfElements];
	...
	void opAssign(X)(X slice) if (!isDynamicArray!X) {
		reset();
		add(slice[]);
	}
	...
	bool opEquals()(auto ref Vector!(T) r) {
...
scope(exit){
	if(gVectorsCreated!=gVectorsDestroyed){
		import std.stdio;
