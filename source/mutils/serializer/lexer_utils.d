...
void serializeNumberToken(bool load, Container)(ref TokenData token, ref Container con) {
	static if (load == true) {
		...
		if (con.length > 0 && con[0] == '.') {
			...
			double num = stringToLong(firstPart) + cast(double) stringToLong(
					secondPart) / (10 ^^ secondPart.length);
...
struct TokenData {
	...
	bool isString(string ss) {
		return (type == StandardTokens.comment_line || type == StandardTokens.comment_multiline
				...
				|| type == StandardTokens.string_ || type == StandardTokens.white) && str == ss;
	}
	...
	void opAssign(T)(T el)
			if (isIntegral!T || isFloatingPoint!T || is(T == string)
				|| is(Unqual!T == char) || is(T == bool)) {
		alias TP = Unqual!T;
	...
	string toString() {
		...
		case StandardTokens.character:
			return format("TK(%5s, '%s', %s, %s)",
					cast(StandardTokens) type, ch, line, column);
		...
		case StandardTokens.comment_multiline:
			return format("TK(%5s, \"%s\", %s, %s)",
					cast(StandardTokens) type, str, line, column);
		case StandardTokens.double_:
			return format("TK(%5s, %s, %s, %s)",
					cast(StandardTokens) type, double_, line, column);
		case StandardTokens.long_:
			return format("TK(%5s, %s, %s, %s)",
					cast(StandardTokens) type, long_, line, column);
		default:
