	...
	template getImpl() {
		auto getImpl() {
			foreach (i, Key; Elements) {
				static if (i % 2 == 0 && ((__traits(compiles, T == Key)
						&& T == Key) || (__traits(compiles, is(T == Key)) && is(T == Key)))) { //even elements are keys
					struct Returner {
...
unittest {
	alias myMap = CTMap!(6, int, long, 12, 18, 23);
...
unittest {
	alias myMap = CTMap!("str1", 8, "str2", 12, "str3", 23);
