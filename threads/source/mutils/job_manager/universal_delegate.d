...
import std.traits : isDelegate, isFunctionPointer, Parameters, ParameterStorageClass, ParameterStorageClassTuple, ReturnType;
...
struct UniversalDelegate(Delegate) {
	static assert(is(Delegate == function) || isFunctionPointer!Delegate
			|| isDelegate!Delegate, "Provided type has to be: delegate, function, function pointer");
...
private template getDelegateArgumentsSave(Delegate) {
	alias getDelegateArgumentsSave = getDelegateArgumentsSaveImpl!(
			ParameterStorageClassTuple!Delegate, Parameters!Delegate).result;
}

private template getDelegateArgumentsSaveImpl(args...) if (args.length % 2 == 0) {
...
unittest {
	...
	TestTmp test;
	ulong returnByRef;
	auto universalDelegate = makeUniversalDelegate!(typeof(&test.add))(&test.add,
			2, 2, returnByRef);
