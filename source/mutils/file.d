...
import core.stdc.stdio : fclose, FILE, fopen, fread, fseek, ftell, fwrite,
	removeFileC = remove, SEEK_END, SEEK_SET;
...
struct FileWatcher {
	...
	bool watchFile(const(char)[] path, EventDelegate del) {
		...
		WatchedFileInfo* info = &watchedFiles.getInsertDefault(
				Vector!(char)(cast(char[]) path), WatchedFileInfo(timestamp));
...
unittest {
	...
	FileWatcher.instance.watchFile("source/app.d", &watch);
	FileWatcher.instance.watchFile("source/app.d", &watch);
	assert(FileWatcher.instance.watchedFiles.get(Vector!(char)(cast(char[]) "source/app.d"))
			.dels.length == 1);
